  ...
  ExportManager.prototype.getCodeListFile = function(ccl) {
    ...
    for (var m in ccl) {
      ...
      if (m == 'NonNavigableSpace') {
        ...
        for (var c in ccl[m]['usage']) txt += path + 'function,' + c + ',' + ccl[m]['usage'][c] + '\n';
      }
    }

    var element = document.createElement('a');
    element.setAttribute('href', 'data:application/csv;charset=utf-8,' + encodeURIComponent(txt));
    ...
    for (var i = 0; i < window.document.getElementById('go-factory-download').children[1].children.length - 1; i++) {
      window.document.getElementById('go-factory-download').children[1].removeChild(window.document.getElementById('go-factory-download').children[1].children[1]);
    }
    window.document.getElementById('go-factory-download').children[1].appendChild(element)
  }
    ...
    for (var key in properties) {
      ...
      if (properties[key].extend.moduleType != "") {
        if (properties[key].extend.featureType == 'NonNavigableSpace') {
          ...
          codeList['nonnavigableSpace'][cells[id].properties.obstacleType] =
            cl.getDesc(['NonNavigableSpace'], cells[id].properties.obstacleType);
          ...
          codeList[cells[id].type]['function'][cells[id].properties.function] =
            cl.getDesc([cells[id].type, 'function'], cells[id].properties.function);

          codeList[cells[id].type]['class'][cells[id].properties.class] =
            cl.getDesc([cells[id].type, 'class'], cells[id].properties.class);

          codeList[cells[id].type]['usage'][cells[id].properties.usage] =
            cl.getDesc([cells[id].type, 'function'], cells[id].properties.usage);
